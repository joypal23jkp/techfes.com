...
const addNextItem = () => {
  if (selectedContentIndex.value === contents.length - 1) {
    selectedContentIndex.value = 0;
  } else {
    selectedContentIndex.value = selectedContentIndex.value + 1;
  }
}

const addPrevItem = () => {
  if (selectedContentIndex.value === 0) {
    selectedContentIndex.value = contents.length;
  } else {
    selectedContentIndex.value = selectedContentIndex.value - 1;
  }
}
...
<div class="container mx-auto py-[100px] flex flex-center ">
  <div class="bg-[url(@/assets/img/bg.jpg)] bg-repeat-x bg-contain bg-[position-y: 90%] flex flex-col lg:flex-row gap-16 lg:gap-4">
    <div class="w-full lg:w-1/2">
      ...
      <div class="flex w-full pt-8">
        ...
        <div class="w-1/2">
          <h2 class="text-4xl font-bold text-primary-400">{{ section.projectCount }}+</h2>
          Completed Projects
        </div>
      </div>
    </div>
    <section class="flex">
      <div class="relative">
        <CommonCard
            class-name="w-full lg:!w-[570px] h-[500px] flex flex-center border-l border-b"
        >
          <figure class="rounded-xl p-8">
            ...
            <figcaption class="pt-6 text-center space-y-4">
              <blockquote>
                <p class="text-lg">{{ contents[selectedContentIndex].comment }}</p>
              </blockquote>
              <div>
                <div class="text-sky-500 font-bold text-2xl dark:text-sky-400">{{ contents[selectedContentIndex].name }}</div>
                <div class="text-slate-700 dark:text-slate-500">{{ contents[selectedContentIndex].designation }}</div>
              </div>
            </figcaption>
          </figure>
        </CommonCard>
        <NuxtLink :to="section.link" class="w-[180px] float-right text-base leading-[26px] text-primary-400 flex-center p-2 my-4 cursor-pointer hover:font-bold transition-all focus:outline-none">
          {{ 'See More' }}
        ...
        <svg class="absolute top-[56px] left-0 hidden lg:block" width="70" height="71" viewBox="0 0 70 71" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="w-[10%] flex gap-4 p-4 lg:hidden">
          <CommonBtnPrimary name="<" class-name="p-4 rounded-full" @onButtonClicked="addNextItem">
            <template #icon>
              <span></span>
            </template>
          </CommonBtnPrimary>
          <CommonBtnPrimary name=">" class-name="p-4 rounded-full" @onButtonClicked="addPrevItem">
            <template #icon>
              <span></span>
            </template>
          </CommonBtnPrimary>
        ...
        <ul class="mt-10">
          <li v-for="(item, index) in contents" :key="item.toString()" class="mt-4">
            <img
                ...
                :width="(80 - (index * 10))"
                :height="(80 - (index * 10))"
                @click.prevent="selectedContentIndex = index"
            >
          </li>
        </ul>
      </div>
